export default {
  title: 'TheMarker',
  url: 'www.haaretz.co.il',
  extraLinks: [
    {
      contentId: '1',
      contentName: 'aa',
      href: 'www.google.com',
      inputTemplate: 'com.tm.Link',
      linkText: 'First Link',
      toolTip: '',
    },
    {
      contentId: '2',
      contentName: 'bb',
      href: 'www.google.com',
      inputTemplate: 'com.tm.Link',
      linkText: 'Second Link',
      toolTip: '',
    },
    {
      contentId: '3',
      contentName: 'cc',
      href: 'www.google.com',
      inputTemplate: 'com.tm.Link',
      linkText: 'Third Link',
      toolTip: '',
    },
    {
      contentId: '4',
      contentName: 'dd',
      href: 'www.google.com',
      inputTemplate: 'com.tm.Link',
      linkText: 'Fourth Link',
      toolTip: '',
    },
  ],
  items: [
    {
      image: {
        inputTemplate: 'com.tm.Image',
        viewMode: null,
        accessibility: 'אסף (משמאל) לצד שתי מגישות ההצעה הנוספות',
        title: 'אסף (משמאל) לצד שתי מגישות ההצעה הנוספות',
        credit: 'האוסף של רוז אסף',
        aspects: {
          full: {
            width: 768,
            height: 1024,
          },
        },
        isAnimated: null,
        contentId: '1.6724547',
        imgArray: [
          {
            imgName: 'image/1693764670.jpg',
            version: '1544208664',
            aspects: {
              full: {
                width: 768,
                height: 1024,
              },
            },
          },
        ],
        imageType: 'image',
        contentName: 'אסף (משמאל) לצד שתי מגישות ההצעה הנוספות',
      },
      contentId: '7.7871808',
      representedContent: '1.6724526',
      title: 'במקום הגנת סייבר: "מריונטה של השב"כ" ב-200 מיליון שקל',
      path: 'https://www.haaretz.co.il/news/politics/.premium-1.6724526',
      titleMobile:
        "ביוזמת ישראלית, מועצת הסטודנטים באונ' ניו יורק הצביעה בעד הצעת BDS",
      hash: 'A2CD2663600E560DEF070DA8735691B3',
      commentsCounts: 66,
      lastUpdate: '2018-12-12T15:24:20.082Z',
      authors: [
        {
          image: {
            photographer: 'none',
            viewMode: 'FullColumnWithVerticalImage',
            accessibility: 'עידו אפרתי',
            credit: 'דודו בכר',
            title: 'עידו אפרתי',
            aspects: {
              vertical: {
                x: 488,
                y: 0,
                width: 1212,
                height: 1420,
              },
              regular: {
                x: 127,
                y: 36,
                width: 1910,
                height: 1432,
              },
              headline: {
                x: 64,
                y: 87,
                width: 2041,
                height: 1187,
              },
              square: {
                x: 365,
                y: 28,
                width: 1442,
                height: 1440,
              },
              full: {
                width: 2200,
                height: 1468,
              },
            },
            imgArray: [
              {
                imgName: 'image/3540675819.png',
                version: '1530698156',
                aspects: {
                  vertical: {
                    x: 488,
                    y: 0,
                    width: 1212,
                    height: 1420,
                  },
                  regular: {
                    x: 127,
                    y: 36,
                    width: 1910,
                    height: 1432,
                  },
                  headline: {
                    x: 64,
                    y: 87,
                    width: 2041,
                    height: 1187,
                  },
                  square: {
                    x: 365,
                    y: 28,
                    width: 1442,
                    height: 1440,
                  },
                  full: {
                    width: 2200,
                    height: 1468,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.3010647',
            contentName: 'עידו אפרתי',
          },
          inputTemplate: 'com.tm.Author',
          contentId: '1.935802',
          contentName: 'עידו אפרתי',
        },
      ],
    },
    {
      id: 'haaretz.co.il.mobile_web.box1.article',
      className: 'h-hidden ad--c h-mb h-tac c-ad',
      audianceTarget: 'all',
      hideOnSite: false,
      inputTemplate: 'com.polobase.DfpBannerElement',
      contentId: '7.4027921',
      contentName: 'Haaretz.co.il.Mobile_web.Box1.Article',
    },
    {
      image: {
        inputTemplate: 'com.tm.Image',
        viewMode: null,
        accessibility:
          "הלוויית ג'ורג' בוש האב. מימין: ג'ימי קרטר, הילרי וביל קלינטון, מישל וברק אובמה, מלניה ודונלד טראמפ",
        title:
          "הלוויית ג'ורג' בוש האב. מימין: ג'ימי קרטר, הילרי וביל קלינטון, מישל וברק אובמה, מלניה ודונלד טראמפ",
        credit: 'Carolyn Kaster/אי־פי ',
        aspects: {
          vertical: {
            x: 488,
            y: 8,
            width: 1247,
            height: 1460,
          },
          regular: {
            x: 262,
            y: 16,
            width: 1938,
            height: 1452,
          },
          belgrade: {
            x: 0,
            y: 107,
            width: 2196,
            height: 690,
          },
          headline: {
            x: 0,
            y: 0,
            width: 2192,
            height: 1274,
          },
          landscape: {
            x: 0,
            y: 4,
            width: 2164,
            height: 932,
          },
          square: {
            x: 663,
            y: 16,
            width: 1454,
            height: 1452,
          },
          full: {
            width: 2200,
            height: 1468,
          },
        },
        isAnimated: null,
        contentId: '1.6724052',
        imgArray: [
          {
            imgName: 'image/3137786967.jpg',
            version: '1544152984',
            aspects: {
              vertical: {
                x: 488,
                y: 8,
                width: 1247,
                height: 1460,
              },
              regular: {
                x: 262,
                y: 16,
                width: 1938,
                height: 1452,
              },
              belgrade: {
                x: 0,
                y: 107,
                width: 2196,
                height: 690,
              },
              headline: {
                x: 0,
                y: 0,
                width: 2192,
                height: 1274,
              },
              landscape: {
                x: 0,
                y: 4,
                width: 2164,
                height: 932,
              },
              square: {
                x: 663,
                y: 16,
                width: 1454,
                height: 1452,
              },
              full: {
                width: 2200,
                height: 1468,
              },
            },
          },
        ],
        imageType: 'image',
        contentName:
          "הלוויית ג'ורג' בוש האב. מימין: ג'ימי קרטר, הילרי וביל קלינטון, מישל וברק אובמה, מלניה ודונלד טראמפ",
      },
      contentId: '7.7871816',
      representedContent: '1.6723848',
      title:
        'מהקונספירציות חובקות העולם של טראמפ אפילו מנהיגים מתים לא חומקים2',
      path: 'https://www.haaretz.co.il/news/world/america/.premium-1.6723848',
      titleMobile:
        'מהקונספירציות חובקות העולם של טראמפ אפילו מנהיגים מתים לא חומקים',
      hash: '759DEE56C720A5B28C9DBB481DF76341',
      commentsCounts: 66,
      lastUpdate: '2018-12-12T15:24:20.082Z',
      authors: [
        {
          image: {
            photographer: 'none',
            viewMode: 'regularModeBigImage',
            accessibility: 'איתי שטרן',
            credit: 'תומר אפלבאום',
            title: 'איתי שטרן',
            aspects: {
              vertical: {
                x: 90,
                y: 48,
                width: 317,
                height: 374,
              },
              regular: {
                x: 71,
                y: 69,
                width: 366,
                height: 275,
              },
              headline: {
                x: 0,
                y: 24,
                width: 2140,
                height: 1242,
              },
              landscape: {
                x: 0,
                y: 32,
                width: 2200,
                height: 948,
              },
              square: {
                x: 524,
                y: 0,
                width: 1017,
                height: 1016,
              },
              full: {
                width: 2200,
                height: 1468,
              },
            },
            imgArray: [
              {
                imgName: 'image/237918199.jpg',
                version: '1514360450',
                aspects: {
                  vertical: {
                    x: 90,
                    y: 48,
                    width: 317,
                    height: 374,
                  },
                  regular: {
                    x: 71,
                    y: 69,
                    width: 366,
                    height: 275,
                  },
                  headline: {
                    x: 0,
                    y: 24,
                    width: 2140,
                    height: 1242,
                  },
                  landscape: {
                    x: 0,
                    y: 32,
                    width: 2200,
                    height: 948,
                  },
                  square: {
                    x: 524,
                    y: 0,
                    width: 1017,
                    height: 1016,
                  },
                  full: {
                    width: 2200,
                    height: 1468,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.2805676',
            contentName: 'איתי שטרן',
          },
          inputTemplate: 'com.tm.Author',
          contentId: '1.1450',
          contentName: 'איתי שטרן',
        },
      ],
    },
    {
      image: {
        inputTemplate: 'com.tm.Image',
        viewMode: null,
        accessibility: 'קובי אשרת',
        title: 'קובי אשרת',
        credit: 'ינאי יחיאל',
        aspects: {
          landscape: {
            x: 0,
            y: 52,
            width: 895,
            height: 386,
          },
          square: {
            x: 151,
            y: 6,
            width: 593,
            height: 594,
          },
          full: {
            width: 900,
            height: 600,
          },
        },
        isAnimated: null,
        contentId: '1.6722140',
        imgArray: [
          {
            imgName: 'image/182418768.jpg',
            version: '1544073092',
            aspects: {
              landscape: {
                x: 0,
                y: 52,
                width: 895,
                height: 386,
              },
              square: {
                x: 151,
                y: 6,
                width: 593,
                height: 594,
              },
              full: {
                width: 900,
                height: 600,
              },
            },
          },
        ],
        imageType: 'image',
        contentName: 'קובי אשרת',
      },
      contentId: '7.7871765',
      representedContent: '1.6722139',
      exclusive: 'AAAAAAA',
      title: '"הגעתי לגיל שבו אבא שלי נפטר. אני לא יכול להתחמק מהמחשבות"3',
      path: 'https://www.haaretz.co.il/magazine/panim/1.6722139',
      titleMobile: '"הגעתי לגיל שבו אבא שלי נפטר. אני לא יכול להתחמק מהמחשבות"',
      hash: '7B7C0E76D94DC4290B579A1B354ECB43',
      commentsCounts: 66,
      lastUpdate: '2018-12-12T15:24:20.082Z',
      authors: [
        {
          contentName: 'רבקה זיוון',
        },
      ],
    },
    {
      image: {
        inputTemplate: 'com.tm.Image',
        viewMode: null,
        accessibility: 'גור. טריילר מלך האריות',
        title:
          'מתוך הטריילר ל"מלך האריות" החדש. למרות החדשנות, המיינסטרים ההוליוודי עדיין שולט',
        credit: 'מתוך ערוץ היוטיוב',
        aspects: {
          headline: {
            x: 55,
            y: 3,
            width: 1853,
            height: 1077,
          },
          landscape: {
            x: 0,
            y: 158,
            width: 1914,
            height: 826,
          },
          square: {
            x: 516,
            y: 9,
            width: 1071,
            height: 1071,
          },
          full: {
            width: 1920,
            height: 1080,
          },
        },
        isAnimated: null,
        contentId: '1.6720456',
        imgArray: [
          {
            imgName: 'image/3893179035.jpg',
            version: '1543998736',
            aspects: {
              headline: {
                x: 55,
                y: 3,
                width: 1853,
                height: 1077,
              },
              landscape: {
                x: 0,
                y: 158,
                width: 1914,
                height: 826,
              },
              square: {
                x: 516,
                y: 9,
                width: 1071,
                height: 1071,
              },
              full: {
                width: 1920,
                height: 1080,
              },
            },
          },
        ],
        imageType: 'image',
        contentName: 'טריילר מלך האריות',
      },
      contentId: '7.7871789',
      representedContent: '1.6720474',
      title:
        'המלך יוצג לראווה בשנה הבאה: למה כל העולם ממתין לסרט שהוא כבר ראה4',
      path:
        'https://www.haaretz.co.il/gallery/cinema/.premium-MAGAZINE-1.6720474',
      titleMobile:
        'המלך יוצג לראווה בשנה הבאה: למה כל העולם ממתין לסרט שהוא כבר ראה',
      hash: 'BF7F8D1905644F89B35C364FB7E06520',
      commentsCounts: 66,
      lastUpdate: '2018-12-12T15:24:20.082Z',
      authors: [
        {
          contentName: 'מירב מורן, TheMarker',
        },
      ],
    },
    {
      image: {
        inputTemplate: 'com.tm.Image',
        viewMode: null,
        accessibility: 'קובי אשרת',
        title: 'קובי אשרת',
        credit: 'ינאי יחיאל',
        aspects: {
          landscape: {
            x: 0,
            y: 52,
            width: 895,
            height: 386,
          },
          square: {
            x: 151,
            y: 6,
            width: 593,
            height: 594,
          },
          full: {
            width: 900,
            height: 600,
          },
        },
        isAnimated: null,
        contentId: '1.6722140',
        imgArray: [
          {
            imgName: 'image/182418768.jpg',
            version: '1544073092',
            aspects: {
              landscape: {
                x: 0,
                y: 52,
                width: 895,
                height: 386,
              },
              square: {
                x: 151,
                y: 6,
                width: 593,
                height: 594,
              },
              full: {
                width: 900,
                height: 600,
              },
            },
          },
        ],
        imageType: 'image',
        contentName: 'קובי אשרת',
      },
      contentId: '7.7871765',
      representedContent: '1.6722139',
      title: '"הגעתי לגיל שבו אבא שלי נפטר. אני לא יכול להתחמק מהמחשבו"5',
      path: 'https://www.haaretz.co.il/magazine/panim/1.6722139',
      titleMobile: '"הגעתי לגיל שבו אבא שלי נפטר. אני לא יכול להתחמק מהמחשבות"',
      hash: '7B7C0E76D94DC4290B579A1B354ECB43',
      commentsCounts: 66,
      lastUpdate: '2018-12-12T15:24:20.082Z',
      authors: [
        {
          image: {
            photographer: 'none',
            viewMode: 'regularModeBigImage',
            accessibility: 'איתי שטרן',
            credit: 'תומר אפלבאום',
            title: 'איתי שטרן',
            aspects: {
              vertical: {
                x: 90,
                y: 48,
                width: 317,
                height: 374,
              },
              regular: {
                x: 71,
                y: 69,
                width: 366,
                height: 275,
              },
              headline: {
                x: 0,
                y: 24,
                width: 2140,
                height: 1242,
              },
              landscape: {
                x: 0,
                y: 32,
                width: 2200,
                height: 948,
              },
              square: {
                x: 524,
                y: 0,
                width: 1017,
                height: 1016,
              },
              full: {
                width: 2200,
                height: 1468,
              },
            },
            imgArray: [
              {
                imgName: 'image/237918199.jpg',
                version: '1514360450',
                aspects: {
                  vertical: {
                    x: 90,
                    y: 48,
                    width: 317,
                    height: 374,
                  },
                  regular: {
                    x: 71,
                    y: 69,
                    width: 366,
                    height: 275,
                  },
                  headline: {
                    x: 0,
                    y: 24,
                    width: 2140,
                    height: 1242,
                  },
                  landscape: {
                    x: 0,
                    y: 32,
                    width: 2200,
                    height: 948,
                  },
                  square: {
                    x: 524,
                    y: 0,
                    width: 1017,
                    height: 1016,
                  },
                  full: {
                    width: 2200,
                    height: 1468,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.2805676',
            contentName: 'איתי שטרן',
          },
          inputTemplate: 'com.tm.Author',
          contentId: '1.1450',
          contentName: 'איתי שטרן',
        },
      ],
    },
  ],
};
