export default {
  title: 'בריאות',
  view: 'Hawking',
  extraLinks: [
    {
      href: 'https://www.haaretz.co.il',
      inputTemplate: 'com.tm.Link',
      contentId: '1.6653506',
      contentName: 'תת מדור 1',
    },
    {
      href: 'http://www.themarker.com',
      linkText: 'תת מדור 2',
      inputTemplate: 'com.tm.Link',
      contentId: '1.6653507',
      contentName: 'תת מדור 2',
    },
  ],
  marketingTeaser: {
    title: 'שיווק בריאות',
    subtitle: 'שיווק בריאות שיווק בריאות',
    href: 'https://promotions.haaretz.co.il/promotions-page/product',
    cta: 'לרכישה',
    inputTemplate: 'com.tm.element.MarketingTeaser',
    contentId: '7.7831979',
    contentName: 'שיווק בריאות',
  },
  hasPagination: false,
  items: [
    {
      path: '/news/health/i-just-asked/.premium-1.6652094',
      image: {
        photographer: 'none',
        accessibility: 'כבל חשמל חשוף ורגליים יחפות',
        credit: 'Aneb / Getty Images IL',
        title:
          'סכנת התחשמלות. הזרם החשמלי עושה את הדרך הקצרה ביותר מנקודת הכניסה לגוף לנקודת היציאה וגורם נזקים',
        aspects: {
          vertical: {
            x: 0,
            y: 0,
            width: 1240,
            height: 1457,
          },
          regular: {
            x: 0,
            y: 0,
            width: 1940,
            height: 1457,
          },
          belgrade: {
            x: 0,
            y: 555,
            width: 2200,
            height: 693,
          },
          headline: {
            x: 0,
            y: 177,
            width: 2200,
            height: 1280,
          },
          landscape: {
            x: 0,
            y: 299,
            width: 2200,
            height: 949,
          },
          square: {
            x: 67,
            y: 0,
            width: 1456,
            height: 1457,
          },
          full: {
            width: 2200,
            height: 1457,
          },
        },
        imgArray: [
          {
            imgName: 'image/1419370526.jpg',
            version: '1542102318',
            aspects: {
              vertical: {
                x: 0,
                y: 0,
                width: 1240,
                height: 1457,
              },
              regular: {
                x: 0,
                y: 0,
                width: 1940,
                height: 1457,
              },
              belgrade: {
                x: 0,
                y: 555,
                width: 2200,
                height: 693,
              },
              headline: {
                x: 0,
                y: 177,
                width: 2200,
                height: 1280,
              },
              landscape: {
                x: 0,
                y: 299,
                width: 2200,
                height: 949,
              },
              square: {
                x: 67,
                y: 0,
                width: 1456,
                height: 1457,
              },
              full: {
                width: 2200,
                height: 1457,
              },
            },
          },
        ],
        imageType: 'image',
        inputTemplate: 'com.tm.Image',
        contentId: '1.6652096',
        contentName: 'התחשמלות',
        kind: 'image',
      },
      kind: 'teaser',
      representedContent: '1.6652094',
      inputTemplate: 'com.tm.TeaserData',
      contentId: '7.7828596',
      publishDate: 1542111207986,
      exclusive: 'אני רק שאלה',
      exclusiveMobile: 'אני רק שאלה',
      title: 'מה קורה בגוף כשמתחשמלים?',
      titleMobile: 'מה קורה בגוף כשמתחשמלים?',
      commentsCounts: 1,
      authors: [
        {
          inputTemplate: 'com.tm.Author',
          contentId: '1.6596928',
          contentName: 'נטע סופר צור',
        },
        {
          contentName: 'המחשמלים',
        },
      ],
    },
    {
      path: '/news/health/research/1.6640598',
      kind: 'teaser',
      representedContent: '1.6640598',
      inputTemplate: 'com.tm.TeaserData',
      contentId: '1.6640598',
      exclusive: 'תרופה',
      exclusiveMobile: 'תרופה',
      title: 'תרופה לשלוש מחלות יתום קיבלה אישור FDA מהיר אחרי ניסוי קצר ומוצלח',
      titleMobile: 'תרופה לשלוש מחלות יתום קיבלה אישור FDA מהיר אחרי ניסוי קצר ומוצלח',
      commentsCounts: 2,
    },
    {
      viewModes: {
        viewModeHtzMobile: 'tadmitMobile',
        viewModeTmMobile: 'tadmitMobile',
        viewModeHtz: 'resp282',
        viewModeJson: 'narrowCommercial',
      },
      banners: [
        {
          priority: 2,
          text: 'רשימת צפיה: חמישה סרטים חדשים שאתם חייבים לראות החודש',
          link: 'http://rmkz.themarker.com/gampad/clk?id=4882652095&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: true,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'פופקורן',
            credit: 'Photo by Christian Wiediger on Unsplash',
            aspects: {
              full: {
                width: 2448,
                height: 1632,
              },
            },
            imgArray: [
              {
                imgName: 'image/1910700103.jpg',
                version: '1544095732',
                aspects: {
                  full: {
                    width: 2448,
                    height: 1632,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6722745',
            contentName: 'פופקורן',
          },
          advertiserCamp: 'יונייטד קינג',
          percentage: 100,
          minRange: 0,
          maxRange: 100,
          chance: 1,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7869851',
          contentName: 'יונייטד קינג',
        },
        {
          priority: 2,
          text: 'אחת ולתמיד: איפה יותר כדאי להשקיע את הכסף, בארץ או בחו"ל?',
          link: 'http://rmkz.themarker.com/gampad/clk?id=4876058760&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: true,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'השקעות',
            aspects: {
              full: {
                width: 140,
                height: 103,
              },
            },
            imgArray: [
              {
                imgName: 'image/1133197235.gif',
                version: '1543410666',
                aspects: {
                  full: {
                    width: 140,
                    height: 103,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6698601',
            contentName: 'השקעות',
          },
          advertiserCamp: 'SDB',
          percentage: 100,
          minRange: 100,
          maxRange: 200,
          chance: 0.5,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7855587',
          contentName: 'SDB - השקעות בארץ מול חו"ל ',
        },
        {
          priority: 2,
          text: 'המודל שינצח את WeWork: מהפיכת המשרדים הקטנים ',
          link: 'http://rmkz.themarker.com/gampad/clk?id=4876507391&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: true,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'תמונת הדמיה ',
            credit: '© Chris Kelleher | Dreamstime.com',
            title: 'במתחם החדש יוצג סטנדרט חדש של משרדים',
            aspects: {
              full: {
                width: 2119,
                height: 1414,
              },
            },
            imgArray: [
              {
                imgName: 'image/2667438960.jpg',
                version: '1542805450',
                aspects: {
                  full: {
                    width: 2119,
                    height: 1414,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6674360',
            contentName: 'תמונת הדמיה',
          },
          advertiserCamp: 'בריטניה ישראל',
          percentage: 100,
          minRange: 200,
          maxRange: 300,
          chance: 0.333,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7855230',
          contentName: 'בריטניה ישראל',
        },
        {
          priority: 2,
          text: 'מגלים את אמריקה: חידון חדש על ארץ האפשרויות הבלתי מוגבלות',
          link: 'http://rmkz.themarker.com/gampad/clk?id=4874869122&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: true,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'פסל החירות',
            credit: 'פרסומת',
            aspects: {
              vertical: {
                x: 401,
                y: 0,
                width: 1390,
                height: 1632,
              },
              regular: {
                x: 0,
                y: 0,
                width: 2175,
                height: 1632,
              },
              belgrade: {
                x: 0,
                y: 410,
                width: 2448,
                height: 772,
              },
              headline: {
                x: 0,
                y: 110,
                width: 2448,
                height: 1425,
              },
              landscape: {
                x: 0,
                y: 221,
                width: 2448,
                height: 1054,
              },
              square: {
                x: 234,
                y: 0,
                width: 1632,
                height: 1632,
              },
              full: {
                width: 2448,
                height: 1632,
              },
            },
            imgArray: [
              {
                imgName: 'image/534517042.jpg',
                version: '1543328436',
                aspects: {
                  vertical: {
                    x: 401,
                    y: 0,
                    width: 1390,
                    height: 1632,
                  },
                  regular: {
                    x: 0,
                    y: 0,
                    width: 2175,
                    height: 1632,
                  },
                  belgrade: {
                    x: 0,
                    y: 410,
                    width: 2448,
                    height: 772,
                  },
                  headline: {
                    x: 0,
                    y: 110,
                    width: 2448,
                    height: 1425,
                  },
                  landscape: {
                    x: 0,
                    y: 221,
                    width: 2448,
                    height: 1054,
                  },
                  square: {
                    x: 234,
                    y: 0,
                    width: 1632,
                    height: 1632,
                  },
                  full: {
                    width: 2448,
                    height: 1632,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6696870',
            contentName: 'פסל החירות',
          },
          advertiserCamp: 'פגסוס',
          percentage: 100,
          minRange: 300,
          maxRange: 400,
          chance: 0.25,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7853561',
          contentName: 'פגסוס חידון ארה"ב 1.6673930',
        },
        {
          priority: 2,
          text: 'תשכחו מחוויות של תרמילאים על חדרים מטונפים: ההוסטלים פורחים',
          link: 'http://rmkz.themarker.com/gampad/clk?id=4873010295&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: true,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'הוסטל',
            credit: 'Unsplash',
            title: 'הדור החדש של הנוודים הדיגיטליים יכולים לטייל ברחבי העולם, בעיקר במדינות המאפשרות מחיה בתקציב נמוך, תוך שילוב עבודה ',
            aspects: {
              full: {
                width: 2447,
                height: 1633,
              },
            },
            imgArray: [
              {
                imgName: 'image/2724716702.jpg',
                version: '1542698407',
                aspects: {
                  full: {
                    width: 2447,
                    height: 1633,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6673423',
            contentName: 'הוסטל',
          },
          advertiserCamp: 'SDB',
          percentage: 100,
          minRange: 400,
          maxRange: 500,
          chance: 0.2,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7848287',
          contentName: 'SDB - הוסטלים',
        },
        {
          priority: 2,
          text: 'חוויות מהגטו הקנדי: האם זו תהיה ההתנצלות הבאה של טרודו?',
          link: 'http://rmkz.themarker.com/gampad/clk?id=4870558777&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: true,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'ג\'סטין טרודו',
            credit: 'פרסומת',
            aspects: {
              vertical: {
                x: 54,
                y: 0,
                width: 409,
                height: 480,
              },
              regular: {
                x: 0,
                y: 0,
                width: 640,
                height: 480,
              },
              belgrade: {
                x: 0,
                y: 12,
                width: 960,
                height: 302,
              },
              headline: {
                x: 0,
                y: 0,
                width: 826,
                height: 480,
              },
              landscape: {
                x: 0,
                y: 0,
                width: 960,
                height: 414,
              },
              square: {
                x: 26,
                y: 0,
                width: 480,
                height: 480,
              },
              full: {
                width: 960,
                height: 480,
              },
            },
            imgArray: [
              {
                imgName: 'image/2737274333.jpg',
                version: '1542896361',
                aspects: {
                  vertical: {
                    x: 54,
                    y: 0,
                    width: 409,
                    height: 480,
                  },
                  regular: {
                    x: 0,
                    y: 0,
                    width: 640,
                    height: 480,
                  },
                  belgrade: {
                    x: 0,
                    y: 12,
                    width: 960,
                    height: 302,
                  },
                  headline: {
                    x: 0,
                    y: 0,
                    width: 826,
                    height: 480,
                  },
                  landscape: {
                    x: 0,
                    y: 0,
                    width: 960,
                    height: 414,
                  },
                  square: {
                    x: 26,
                    y: 0,
                    width: 480,
                    height: 480,
                  },
                  full: {
                    width: 960,
                    height: 480,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6678938',
            contentName: 'ג\'סטין טרודו',
          },
          advertiserCamp: 'בית אבי חי',
          percentage: 100,
          minRange: 500,
          maxRange: 600,
          chance: 0.166,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7845784',
          contentName: 'בית אבי חי גטו קנדי 1.6678794',
        },
        {
          priority: 2,
          text: 'סיפורה של הקהילה היהודית במרוקו',
          link: 'http://rmkz.themarker.com/gampad/clk?id=4867792048&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: true,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'כמו במקומות אחרים בעולם, היו היהודים תמיד כלי ביד השליט',
            credit: 'פרסומת',
            aspects: {
              vertical: {
                x: 472,
                y: 0,
                width: 1389,
                height: 1632,
              },
              regular: {
                x: 525,
                y: 591,
                width: 1389,
                height: 1041,
              },
              belgrade: {
                x: 0,
                y: 860,
                width: 2448,
                height: 772,
              },
              headline: {
                x: 278,
                y: 547,
                width: 1866,
                height: 1085,
              },
              landscape: {
                x: 0,
                y: 578,
                width: 2448,
                height: 1054,
              },
              square: {
                x: 609,
                y: 454,
                width: 1177,
                height: 1178,
              },
              full: {
                width: 2448,
                height: 1632,
              },
            },
            imgArray: [
              {
                imgName: 'image/692462821.jpg',
                version: '1542709308',
                aspects: {
                  vertical: {
                    x: 472,
                    y: 0,
                    width: 1389,
                    height: 1632,
                  },
                  regular: {
                    x: 525,
                    y: 591,
                    width: 1389,
                    height: 1041,
                  },
                  belgrade: {
                    x: 0,
                    y: 860,
                    width: 2448,
                    height: 772,
                  },
                  headline: {
                    x: 278,
                    y: 547,
                    width: 1866,
                    height: 1085,
                  },
                  landscape: {
                    x: 0,
                    y: 578,
                    width: 2448,
                    height: 1054,
                  },
                  square: {
                    x: 609,
                    y: 454,
                    width: 1177,
                    height: 1178,
                  },
                  full: {
                    width: 2448,
                    height: 1632,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6673864',
            contentName: 'כמו במקומות אחרים בעולם, היו היהודים תמיד כלי ביד השליט',
          },
          advertiserCamp: 'פגסוס',
          percentage: 100,
          minRange: 600,
          maxRange: 700,
          chance: 0.142,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7840753',
          contentName: 'פגסוס - קהילה יהודית מרוקו',
        },
        {
          priority: 2,
          text: 'לפני שיהיה מאוחר מדי: מה לעשות כשבן משפחה מתמכר להימורים?',
          link: 'http://rmkz.themarker.com/gampad/clk?id=4863405728&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: true,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'צלליות משפחה על רקע שקיעה',
            credit: 'פרסומת',
            aspects: {
              vertical: {
                x: 635,
                y: 0,
                width: 1390,
                height: 1632,
              },
              regular: {
                x: 252,
                y: 0,
                width: 2175,
                height: 1632,
              },
              belgrade: {
                x: 0,
                y: 357,
                width: 2449,
                height: 772,
              },
              headline: {
                x: 0,
                y: 207,
                width: 2449,
                height: 1425,
              },
              landscape: {
                x: 0,
                y: 300,
                width: 2449,
                height: 1054,
              },
              square: {
                x: 503,
                y: 0,
                width: 1633,
                height: 1632,
              },
              full: {
                width: 2449,
                height: 1632,
              },
            },
            imgArray: [
              {
                imgName: 'image/727714191.jpg',
                version: '1542280781',
                aspects: {
                  vertical: {
                    x: 635,
                    y: 0,
                    width: 1390,
                    height: 1632,
                  },
                  regular: {
                    x: 252,
                    y: 0,
                    width: 2175,
                    height: 1632,
                  },
                  belgrade: {
                    x: 0,
                    y: 357,
                    width: 2449,
                    height: 772,
                  },
                  headline: {
                    x: 0,
                    y: 207,
                    width: 2449,
                    height: 1425,
                  },
                  landscape: {
                    x: 0,
                    y: 300,
                    width: 2449,
                    height: 1054,
                  },
                  square: {
                    x: 503,
                    y: 0,
                    width: 1633,
                    height: 1632,
                  },
                  full: {
                    width: 2449,
                    height: 1632,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6656313',
            contentName: 'צלליות משפחה על רקע שקיעה',
          },
          advertiserCamp: 'מפעל הפיס',
          percentage: 100,
          minRange: 700,
          maxRange: 800,
          chance: 0.125,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7832913',
          contentName: 'מפעל הפיס מה לעשות כשאחד מבני המשפחה מתמכר להימורים? 1.6655802',
        },
        {
          priority: 2,
          text: 'משחק המונופול שיפתור את הבעיות הרגולטוריות של ישראל',
          link: 'http://rmkz.themarker.com/gampad/clk?id=4860533226&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: true,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'מונופול השקעות זרות',
            aspects: {
              vertical: {
                x: 701,
                y: 0,
                width: 1390,
                height: 1632,
              },
              regular: {
                x: 273,
                y: 0,
                width: 2175,
                height: 1632,
              },
              belgrade: {
                x: 0,
                y: 463,
                width: 2448,
                height: 772,
              },
              headline: {
                x: 0,
                y: 207,
                width: 2448,
                height: 1425,
              },
              landscape: {
                x: 0,
                y: 304,
                width: 2448,
                height: 1055,
              },
              square: {
                x: 666,
                y: 0,
                width: 1632,
                height: 1632,
              },
              full: {
                width: 2448,
                height: 1632,
              },
            },
            imgArray: [
              {
                imgName: 'image/1400672005.jpg',
                version: '1542117563',
                aspects: {
                  vertical: {
                    x: 701,
                    y: 0,
                    width: 1390,
                    height: 1632,
                  },
                  regular: {
                    x: 273,
                    y: 0,
                    width: 2175,
                    height: 1632,
                  },
                  belgrade: {
                    x: 0,
                    y: 463,
                    width: 2448,
                    height: 772,
                  },
                  headline: {
                    x: 0,
                    y: 207,
                    width: 2448,
                    height: 1425,
                  },
                  landscape: {
                    x: 0,
                    y: 304,
                    width: 2448,
                    height: 1055,
                  },
                  square: {
                    x: 666,
                    y: 0,
                    width: 1632,
                    height: 1632,
                  },
                  full: {
                    width: 2448,
                    height: 1632,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6652738',
            contentName: 'מונופול השקעות זרות',
          },
          advertiserCamp: 'משרד הכלכלה',
          percentage: 100,
          minRange: 800,
          maxRange: 900,
          chance: 0.111,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7828856',
          contentName: 'שולחן עגול מונופול 1.6652093',
        },
        {
          priority: 2,
          text: 'הכירו את האנשים שילמדו אתכם כל מה שצריך לדעת על מכירות אונליין',
          link: 'http://pubads.g.doubleclick.net/gampad/clk?id=4859376402&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: false,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'אלעד גולדנברג ושגיא סטוניס, מקימי מרכז ההכשרה Digital IL ',
            aspects: {
              vertical: {
                x: 803,
                y: 0,
                width: 1389,
                height: 1632,
              },
              regular: {
                x: 273,
                y: 0,
                width: 2175,
                height: 1632,
              },
              belgrade: {
                x: 0,
                y: 159,
                width: 2448,
                height: 772,
              },
              headline: {
                x: 0,
                y: 146,
                width: 2448,
                height: 1424,
              },
              landscape: {
                x: 0,
                y: 146,
                width: 2448,
                height: 1054,
              },
              square: {
                x: 816,
                y: 0,
                width: 1632,
                height: 1632,
              },
              full: {
                width: 2448,
                height: 1632,
              },
            },
            imgArray: [
              {
                imgName: 'image/763555896.jpg',
                version: '1542108483',
                aspects: {
                  vertical: {
                    x: 803,
                    y: 0,
                    width: 1389,
                    height: 1632,
                  },
                  regular: {
                    x: 273,
                    y: 0,
                    width: 2175,
                    height: 1632,
                  },
                  belgrade: {
                    x: 0,
                    y: 159,
                    width: 2448,
                    height: 772,
                  },
                  headline: {
                    x: 0,
                    y: 146,
                    width: 2448,
                    height: 1424,
                  },
                  landscape: {
                    x: 0,
                    y: 146,
                    width: 2448,
                    height: 1054,
                  },
                  square: {
                    x: 816,
                    y: 0,
                    width: 1632,
                    height: 1632,
                  },
                  full: {
                    width: 2448,
                    height: 1632,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6652376',
            contentName: 'אלעד גולדנברג ושגיא סטוניס, מקימי מרכז ההכשרה Digital IL ',
          },
          advertiserCamp: 'דיגיטל IL',
          percentage: 100,
          minRange: 900,
          maxRange: 1000,
          chance: 0.1,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7828511',
          contentName: 'דיגיטל IL תמונות יזמים 1.6640681',
        },
        {
          priority: 2,
          text: 'איך מביימים שיר? יצאנו לבדוק',
          link: 'http://rmkz.themarker.com/gampad/clk?id=4855272201&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: true,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'צילום מסך מתוך "דבש שחור" ',
            aspects: {
              vertical: {
                x: 133,
                y: 0,
                width: 366,
                height: 430,
              },
              regular: {
                x: 0,
                y: 0,
                width: 571,
                height: 428,
              },
              belgrade: {
                x: 0,
                y: 120,
                width: 571,
                height: 179,
              },
              headline: {
                x: 0,
                y: 51,
                width: 571,
                height: 331,
              },
              landscape: {
                x: 0,
                y: 85,
                width: 571,
                height: 246,
              },
              square: {
                x: 70,
                y: 0,
                width: 430,
                height: 430,
              },
              full: {
                width: 571,
                height: 430,
              },
            },
            imgArray: [
              {
                imgName: 'image/2262240543.png',
                version: '1541687732',
                aspects: {
                  vertical: {
                    x: 133,
                    y: 0,
                    width: 366,
                    height: 430,
                  },
                  regular: {
                    x: 0,
                    y: 0,
                    width: 571,
                    height: 428,
                  },
                  belgrade: {
                    x: 0,
                    y: 120,
                    width: 571,
                    height: 179,
                  },
                  headline: {
                    x: 0,
                    y: 51,
                    width: 571,
                    height: 331,
                  },
                  landscape: {
                    x: 0,
                    y: 85,
                    width: 571,
                    height: 246,
                  },
                  square: {
                    x: 70,
                    y: 0,
                    width: 430,
                    height: 430,
                  },
                  full: {
                    width: 571,
                    height: 430,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6635836',
            contentName: 'צילום מסך מתוך "דבש שחור" ',
          },
          advertiserCamp: 'בית אבי חי',
          percentage: 100,
          minRange: 1000,
          maxRange: 1100,
          chance: 0.09,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7821307',
          contentName: 'בית אבי חי איך מביימים שיר 1.6635691',
        },
        {
          priority: 2,
          text: 'אדם שיש לו בעיה של הימורים, בדרך כלל לא יודה בזה',
          link: 'http://rmkz.themarker.com/gampad/clk?id=4854959928&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: true,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'משחק פוקר',
            credit: 'פרסומת',
            aspects: {
              full: {
                width: 2453,
                height: 1630,
              },
            },
            imgArray: [
              {
                imgName: 'image/123936711.jpg',
                version: '1541667072',
                aspects: {
                  full: {
                    width: 2453,
                    height: 1630,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6634766',
            contentName: 'משחק פוקר',
          },
          advertiserCamp: 'מפעל הפיס',
          percentage: 100,
          minRange: 1100,
          maxRange: 1200,
          chance: 0.083,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7820477',
          contentName: 'מפעל הפיס "אסור לשתף פעולה עם ההכחשה" 1.6631471',
        },
        {
          priority: 2,
          text: 'סרט חדש על הסופר דוד פוגל חושף צדדים לא מוכרים בדמותו',
          link: 'http://pubads.g.doubleclick.net/gampad/clk?id=4850263363&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: false,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'דוד פוגל',
            credit: 'פרסומת',
            aspects: {
              vertical: {
                x: 182,
                y: 0,
                width: 408,
                height: 480,
              },
              regular: {
                x: 131,
                y: 0,
                width: 640,
                height: 480,
              },
              belgrade: {
                x: 0,
                y: 38,
                width: 960,
                height: 302,
              },
              headline: {
                x: 61,
                y: 0,
                width: 826,
                height: 480,
              },
              landscape: {
                x: 0,
                y: 0,
                width: 960,
                height: 414,
              },
              square: {
                x: 179,
                y: 0,
                width: 480,
                height: 480,
              },
              full: {
                width: 960,
                height: 480,
              },
            },
            imgArray: [
              {
                imgName: 'image/396561819.jpg',
                version: '1541336721',
                aspects: {
                  vertical: {
                    x: 182,
                    y: 0,
                    width: 408,
                    height: 480,
                  },
                  regular: {
                    x: 131,
                    y: 0,
                    width: 640,
                    height: 480,
                  },
                  belgrade: {
                    x: 0,
                    y: 38,
                    width: 960,
                    height: 302,
                  },
                  headline: {
                    x: 61,
                    y: 0,
                    width: 826,
                    height: 480,
                  },
                  landscape: {
                    x: 0,
                    y: 0,
                    width: 960,
                    height: 414,
                  },
                  square: {
                    x: 179,
                    y: 0,
                    width: 480,
                    height: 480,
                  },
                  full: {
                    width: 960,
                    height: 480,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6617701',
            contentName: 'דוד פוגל',
          },
          advertiserCamp: 'בית אבי חי',
          percentage: 100,
          minRange: 1200,
          maxRange: 1300,
          chance: 0.076,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7812489',
          contentName: 'בית אבי חי - דוד פוגל 1.6617604',
        },
        {
          priority: 2,
          text: 'תרבות, נוף, אדם: לגלות את וייטנאם מחדש',
          link: 'http://pubads.g.doubleclick.net/gampad/clk?id=4843286314&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: false,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'אישה עובדת בשדה אורז בוייטנאם',
            credit: 'Unsplash',
            aspects: {
              vertical: {
                x: 84,
                y: 0,
                width: 1476,
                height: 1732,
              },
              regular: {
                x: 0,
                y: 0,
                width: 2309,
                height: 1732,
              },
              belgrade: {
                x: 0,
                y: 374,
                width: 2309,
                height: 726,
              },
              headline: {
                x: 0,
                y: 342,
                width: 2309,
                height: 1343,
              },
              landscape: {
                x: 0,
                y: 370,
                width: 2309,
                height: 997,
              },
              square: {
                x: 0,
                y: 0,
                width: 1733,
                height: 1732,
              },
              full: {
                width: 2309,
                height: 1732,
              },
            },
            imgArray: [
              {
                imgName: 'image/93379197.jpg',
                version: '1540818286',
                aspects: {
                  vertical: {
                    x: 84,
                    y: 0,
                    width: 1476,
                    height: 1732,
                  },
                  regular: {
                    x: 0,
                    y: 0,
                    width: 2309,
                    height: 1732,
                  },
                  belgrade: {
                    x: 0,
                    y: 374,
                    width: 2309,
                    height: 726,
                  },
                  headline: {
                    x: 0,
                    y: 342,
                    width: 2309,
                    height: 1343,
                  },
                  landscape: {
                    x: 0,
                    y: 370,
                    width: 2309,
                    height: 997,
                  },
                  square: {
                    x: 0,
                    y: 0,
                    width: 1733,
                    height: 1732,
                  },
                  full: {
                    width: 2309,
                    height: 1732,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6603045',
            contentName: 'אישה עובדת בשדה אורז בוייטנאם',
          },
          advertiserCamp: 'פגסוס',
          percentage: 100,
          minRange: 1300,
          maxRange: 1400,
          chance: 0.071,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7802983',
          contentName: 'פגסוס וייטנאם בליל תרבויות 1.6601792',
        },
        {
          priority: 2,
          text: 'התמכרויות בבני הנוער: כך תזהו, תטפלו ותעצרו אותן בזמן',
          link: 'http://pubads.g.doubleclick.net/gampad/clk?id=4836556070&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: false,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'זה גיל שמתאפיין בחיפוש אחר ריגושים, ולכן הגיל הזה מועד להתמכרויות',
            credit: 'פרסומת',
            aspects: {
              vertical: {
                x: 575,
                y: 0,
                width: 1277,
                height: 1500,
              },
              regular: {
                x: 421,
                y: 0,
                width: 1998,
                height: 1500,
              },
              belgrade: {
                x: 0,
                y: 142,
                width: 2666,
                height: 839,
              },
              headline: {
                x: 85,
                y: 0,
                width: 2581,
                height: 1500,
              },
              landscape: {
                x: 0,
                y: 109,
                width: 2666,
                height: 1152,
              },
              square: {
                x: 600,
                y: 0,
                width: 1499,
                height: 1500,
              },
              full: {
                width: 2666,
                height: 1500,
              },
            },
            imgArray: [
              {
                imgName: 'image/1173457501.jpg',
                version: '1540474407',
                aspects: {
                  vertical: {
                    x: 575,
                    y: 0,
                    width: 1277,
                    height: 1500,
                  },
                  regular: {
                    x: 421,
                    y: 0,
                    width: 1998,
                    height: 1500,
                  },
                  belgrade: {
                    x: 0,
                    y: 142,
                    width: 2666,
                    height: 839,
                  },
                  headline: {
                    x: 85,
                    y: 0,
                    width: 2581,
                    height: 1500,
                  },
                  landscape: {
                    x: 0,
                    y: 109,
                    width: 2666,
                    height: 1152,
                  },
                  square: {
                    x: 600,
                    y: 0,
                    width: 1499,
                    height: 1500,
                  },
                  full: {
                    width: 2666,
                    height: 1500,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6593645',
            contentName: 'זה גיל שמתאפיין בחיפוש אחר ריגושים, ולכן הגיל הזה מועד להתמכרויות',
          },
          advertiserCamp: 'מפעל הפיס',
          percentage: 100,
          minRange: 1400,
          maxRange: 1500,
          chance: 0.066,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7797817',
          contentName: 'מפעל הפיס - התמכרויות בבני נוער 1.6577294',
        },
        {
          priority: 2,
          text: 'איך גורמים למשקיעים זרים לבחור בישראל כיעד להשקעה?',
          link: 'http://pubads.g.doubleclick.net/gampad/clk?id=4838347623&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: false,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'משרד של מייקרוסופט',
            credit: 'פרסומת',
            aspects: {
              vertical: {
                x: 240,
                y: 0,
                width: 478,
                height: 561,
              },
              regular: {
                x: 0,
                y: 0,
                width: 748,
                height: 561,
              },
              belgrade: {
                x: 0,
                y: 83,
                width: 748,
                height: 235,
              },
              headline: {
                x: 0,
                y: 35,
                width: 748,
                height: 435,
              },
              landscape: {
                x: 0,
                y: 44,
                width: 748,
                height: 323,
              },
              square: {
                x: 140,
                y: 0,
                width: 561,
                height: 561,
              },
              full: {
                width: 748,
                height: 561,
              },
            },
            imgArray: [
              {
                imgName: 'image/1418197253.jpg',
                version: '1540381841',
                aspects: {
                  vertical: {
                    x: 240,
                    y: 0,
                    width: 478,
                    height: 561,
                  },
                  regular: {
                    x: 0,
                    y: 0,
                    width: 748,
                    height: 561,
                  },
                  belgrade: {
                    x: 0,
                    y: 83,
                    width: 748,
                    height: 235,
                  },
                  headline: {
                    x: 0,
                    y: 35,
                    width: 748,
                    height: 435,
                  },
                  landscape: {
                    x: 0,
                    y: 44,
                    width: 748,
                    height: 323,
                  },
                  square: {
                    x: 140,
                    y: 0,
                    width: 561,
                    height: 561,
                  },
                  full: {
                    width: 748,
                    height: 561,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6591436',
            contentName: 'משרד של מייקרוסופט',
          },
          advertiserCamp: 'משרד הכלכלה',
          percentage: 100,
          minRange: 1500,
          maxRange: 1600,
          chance: 0.062,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7795514',
          contentName: 'משרד הכלכלה גוגל ופייסבוק זו רק ההתחלה 1.6469515',
        },
        {
          priority: 2,
          text: 'ישראל רוצה ללמוד איך למשוך את המשקיעים הסינים',
          link: 'http://pubads.g.doubleclick.net/gampad/clk?id=4838987255&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: false,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'דגל סין על רקע בניין זכוכית',
            credit: 'פרסומת',
            aspects: {
              vertical: {
                x: 0,
                y: 0,
                width: 1204,
                height: 1414,
              },
              regular: {
                x: 0,
                y: 4,
                width: 1880,
                height: 1410,
              },
              belgrade: {
                x: 0,
                y: 470,
                width: 2121,
                height: 669,
              },
              headline: {
                x: 0,
                y: 180,
                width: 2121,
                height: 1234,
              },
              landscape: {
                x: 0,
                y: 268,
                width: 2121,
                height: 913,
              },
              square: {
                x: 0,
                y: 0,
                width: 1414,
                height: 1414,
              },
              full: {
                width: 2121,
                height: 1414,
              },
            },
            imgArray: [
              {
                imgName: 'image/965683585.jpg',
                version: '1540381672',
                aspects: {
                  vertical: {
                    x: 0,
                    y: 0,
                    width: 1204,
                    height: 1414,
                  },
                  regular: {
                    x: 0,
                    y: 4,
                    width: 1880,
                    height: 1410,
                  },
                  belgrade: {
                    x: 0,
                    y: 470,
                    width: 2121,
                    height: 669,
                  },
                  headline: {
                    x: 0,
                    y: 180,
                    width: 2121,
                    height: 1234,
                  },
                  landscape: {
                    x: 0,
                    y: 268,
                    width: 2121,
                    height: 913,
                  },
                  square: {
                    x: 0,
                    y: 0,
                    width: 1414,
                    height: 1414,
                  },
                  full: {
                    width: 2121,
                    height: 1414,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6591424',
            contentName: 'דגל סין על רקע בניין זכוכית',
          },
          advertiserCamp: 'משרד הכלכלה',
          percentage: 100,
          minRange: 1600,
          maxRange: 1700,
          chance: 0.058,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7795506',
          contentName: 'משרד הכלכלה - עם הפנים למזרח 1.6472389',
        },
        {
          priority: 2,
          text: 'האי הקדוש: על ההיסטוריה העקובה מדם של סרי לנקה',
          link: 'http://pubads.g.doubleclick.net/gampad/clk?id=4835841598&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: false,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'אשה טמילית במטע תה בסרי לנקה',
            credit: 'פרסומת',
            aspects: {
              vertical: {
                x: 76,
                y: 0,
                width: 1195,
                height: 1404,
              },
              regular: {
                x: 0,
                y: 0,
                width: 1870,
                height: 1404,
              },
              belgrade: {
                x: 0,
                y: 121,
                width: 2136,
                height: 672,
              },
              headline: {
                x: 0,
                y: 102,
                width: 2136,
                height: 1241,
              },
              landscape: {
                x: 0,
                y: 110,
                width: 2136,
                height: 922,
              },
              square: {
                x: 15,
                y: 0,
                width: 1404,
                height: 1404,
              },
              full: {
                width: 2136,
                height: 1404,
              },
            },
            imgArray: [
              {
                imgName: 'image/3875302059.jpg',
                version: '1540202146',
                aspects: {
                  vertical: {
                    x: 76,
                    y: 0,
                    width: 1195,
                    height: 1404,
                  },
                  regular: {
                    x: 0,
                    y: 0,
                    width: 1870,
                    height: 1404,
                  },
                  belgrade: {
                    x: 0,
                    y: 121,
                    width: 2136,
                    height: 672,
                  },
                  headline: {
                    x: 0,
                    y: 102,
                    width: 2136,
                    height: 1241,
                  },
                  landscape: {
                    x: 0,
                    y: 110,
                    width: 2136,
                    height: 922,
                  },
                  square: {
                    x: 15,
                    y: 0,
                    width: 1404,
                    height: 1404,
                  },
                  full: {
                    width: 2136,
                    height: 1404,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6577534',
            contentName: 'אשה במטע תה בסרי לנקה',
          },
          advertiserCamp: 'פגסוס',
          percentage: 100,
          minRange: 1700,
          maxRange: 1800,
          chance: 0.055,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7791240',
          contentName: 'פגסוס - סרי לנקה 1.6577422',
        },
        {
          priority: 2,
          text: 'שישה טיפים שיעזרו לכם להיפטר מההרגלים הרעים שלכם',
          link: 'http://pubads.g.doubleclick.net/gampad/clk?id=4827916771&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: false,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'קאפקייקס על רקע ורוד',
            credit: 'פרסומת',
            aspects: {
              full: {
                width: 2448,
                height: 1632,
              },
            },
            imgArray: [
              {
                imgName: 'image/2768761444.jpg',
                version: '1539594862',
                aspects: {
                  full: {
                    width: 2448,
                    height: 1632,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6555355',
            contentName: 'קאפקייקס על רקע ורוד',
          },
          advertiserCamp: 'מפעל הפיס',
          percentage: 100,
          minRange: 1800,
          maxRange: 1900,
          chance: 0.052,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7779805',
          contentName: 'מפעל הפיס - 6 צעדים לאחריות',
        },
        {
          priority: 2,
          text: 'המקונג: נהר שמאחד תרבויות, מדינות וגורלות',
          link: 'http://rmkz.themarker.com/gampad/clk?id=4821637800&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: true,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'נהר המקונג, תאילנד',
            credit: 'פרסומת',
            aspects: {
              vertical: {
                x: 0,
                y: 0,
                width: 1204,
                height: 1414,
              },
              regular: {
                x: 0,
                y: 0,
                width: 1884,
                height: 1414,
              },
              belgrade: {
                x: 0,
                y: 535,
                width: 2121,
                height: 669,
              },
              headline: {
                x: 0,
                y: 168,
                width: 2121,
                height: 1235,
              },
              landscape: {
                x: 0,
                y: 409,
                width: 2121,
                height: 913,
              },
              square: {
                x: 0,
                y: 0,
                width: 1414,
                height: 1414,
              },
              full: {
                width: 2121,
                height: 1414,
              },
            },
            imgArray: [
              {
                imgName: 'image/1564040173.jpg',
                version: '1539088157',
                aspects: {
                  vertical: {
                    x: 0,
                    y: 0,
                    width: 1204,
                    height: 1414,
                  },
                  regular: {
                    x: 0,
                    y: 0,
                    width: 1884,
                    height: 1414,
                  },
                  belgrade: {
                    x: 0,
                    y: 535,
                    width: 2121,
                    height: 669,
                  },
                  headline: {
                    x: 0,
                    y: 168,
                    width: 2121,
                    height: 1235,
                  },
                  landscape: {
                    x: 0,
                    y: 409,
                    width: 2121,
                    height: 913,
                  },
                  square: {
                    x: 0,
                    y: 0,
                    width: 1414,
                    height: 1414,
                  },
                  full: {
                    width: 2121,
                    height: 1414,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6545578',
            contentName: 'נהר המקונג, תאילנד',
          },
          advertiserCamp: 'פגסוס',
          percentage: 100,
          minRange: 1900,
          maxRange: 2000,
          chance: 0.05,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7770674',
          contentName: 'פגסוס - נהר המקונג 1.6545384',
        },
        {
          priority: 2,
          text: 'שוב התפרעתם באמזון? זה הזמן לבדוק האם עברתם את הגבול',
          link: 'http://pubads.g.doubleclick.net/gampad/clk?id=4820394372&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: false,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'לוגו של אמזון בסלולרי',
            credit: 'פרסומת',
            aspects: {
              vertical: {
                x: 759,
                y: 0,
                width: 1389,
                height: 1632,
              },
              regular: {
                x: 273,
                y: 0,
                width: 2175,
                height: 1632,
              },
              belgrade: {
                x: 0,
                y: 587,
                width: 2448,
                height: 772,
              },
              headline: {
                x: 0,
                y: 207,
                width: 2448,
                height: 1425,
              },
              landscape: {
                x: 0,
                y: 494,
                width: 2448,
                height: 1054,
              },
              square: {
                x: 816,
                y: 0,
                width: 1632,
                height: 1632,
              },
              full: {
                width: 2448,
                height: 1632,
              },
            },
            imgArray: [
              {
                imgName: 'image/223264209.jpg',
                version: '1539000722',
                aspects: {
                  vertical: {
                    x: 759,
                    y: 0,
                    width: 1389,
                    height: 1632,
                  },
                  regular: {
                    x: 273,
                    y: 0,
                    width: 2175,
                    height: 1632,
                  },
                  belgrade: {
                    x: 0,
                    y: 587,
                    width: 2448,
                    height: 772,
                  },
                  headline: {
                    x: 0,
                    y: 207,
                    width: 2448,
                    height: 1425,
                  },
                  landscape: {
                    x: 0,
                    y: 494,
                    width: 2448,
                    height: 1054,
                  },
                  square: {
                    x: 816,
                    y: 0,
                    width: 1632,
                    height: 1632,
                  },
                  full: {
                    width: 2448,
                    height: 1632,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6535308',
            contentName: 'לוגו של אמזון בסלולרי',
          },
          advertiserCamp: 'מפעל הפיס',
          percentage: 100,
          minRange: 2000,
          maxRange: 2100,
          chance: 0.047,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7768548',
          contentName: 'שאלון הגזמה בכרטיסי אשראי',
        },
        {
          priority: 2,
          text: '3 אנשים מספרים על הרגע בו הבינו שעברו את הגבול',
          link: 'http://pubads.g.doubleclick.net/gampad/clk?id=4816884435&iu=/9401/ClickTracker.Marketing/ClickTracker.Marketing.All',
          linkTarget: '_blank',
          departments: [
            'all',
          ],
          replaceDomainForAdBlocker: false,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'איש מעשן',
            credit: 'פרסומת',
            aspects: {
              vertical: {
                x: 459,
                y: 0,
                width: 1390,
                height: 1633,
              },
              regular: {
                x: 163,
                y: 0,
                width: 2176,
                height: 1633,
              },
              belgrade: {
                x: 0,
                y: 525,
                width: 2449,
                height: 773,
              },
              headline: {
                x: 0,
                y: 207,
                width: 2449,
                height: 1426,
              },
              landscape: {
                x: 0,
                y: 269,
                width: 2449,
                height: 1055,
              },
              square: {
                x: 331,
                y: 0,
                width: 1633,
                height: 1633,
              },
              full: {
                width: 2449,
                height: 1633,
              },
            },
            imgArray: [
              {
                imgName: 'image/3022875702.jpg',
                version: '1538658351',
                aspects: {
                  vertical: {
                    x: 459,
                    y: 0,
                    width: 1390,
                    height: 1633,
                  },
                  regular: {
                    x: 163,
                    y: 0,
                    width: 2176,
                    height: 1633,
                  },
                  belgrade: {
                    x: 0,
                    y: 525,
                    width: 2449,
                    height: 773,
                  },
                  headline: {
                    x: 0,
                    y: 207,
                    width: 2449,
                    height: 1426,
                  },
                  landscape: {
                    x: 0,
                    y: 269,
                    width: 2449,
                    height: 1055,
                  },
                  square: {
                    x: 331,
                    y: 0,
                    width: 1633,
                    height: 1633,
                  },
                  full: {
                    width: 2449,
                    height: 1633,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6530537',
            contentName: 'איש מעשן',
          },
          advertiserCamp: 'מפעל הפיס',
          percentage: 100,
          minRange: 2100,
          maxRange: 2200,
          chance: 0.045,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7763361',
          contentName: 'מפעל הפיס 3 מונולוגים 1.6527643',
        },
      ],
      totalPercentage: 2200,
      inputTemplate: 'com.polobase.ClickTrackerBannersWrapper',
      contentId: '7.7669246',
      contentName: 'תוכן מקודם- עמוד כתבה- מתחת לאאוטבריין (ריאקט)',
    },
    {
      viewModes: {
        viewModeHtzMobile: 'tadmitMobile',
        viewModeTmMobile: 'tadmitMobile',
        viewModeHtz: 'resp282',
        viewModeJson: 'narrowCommercial',
      },
      banners: [
        {
          priority: 2,
          text: 'מגלים את אמריקה: חידון חדש על ארץ האפשרויות הבלתי מוגבלות',
          link: 'http://rmkz.themarker.com/gampad/clk?id=4876715749&iu=/9401/Haaretz.co.il.ClickTracker/Haaretz.co.il.ClickTracker.Button_222x108.3',
          linkTarget: '_blank',
          departments: [
            'haaretz',
          ],
          replaceDomainForAdBlocker: true,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'ון ליר - אירועי דצמבר',
            credit: 'ון ליר - אירועי דצמבר',
            aspects: {
              full: {
                width: 222,
                height: 108,
              },
            },
            imgArray: [
              {
                imgName: 'image/1080014623.jpg',
                version: '1543734339',
                aspects: {
                  full: {
                    width: 222,
                    height: 108,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.6702686',
            contentName: 'ון ליר - אירועי דצמבר',
          },
          advertiserCamp: 'ון ליר - אירועי דצמבר',
          percentage: 100,
          minRange: 0,
          maxRange: 100,
          chance: 1,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7860598',
          contentName: 'ון ליר - דצמבר - קבוע!',
        },
      ],
      totalPercentage: 100,
      inputTemplate: 'com.polobase.ClickTrackerBannersWrapper',
      contentId: '7.3592182',
      contentName: '222/108 מיקום 3 ון ליר',
    },
    {
      viewModes: {
        viewModeHtzMobile: 'tadmitMobile',
        viewModeTmMobile: 'tadmitMobile',
        viewModeHtz: 'resp282',
        viewModeJson: 'narrowCommercial',
      },
      banners: [
        {
          priority: 2,
          text: 'מגלים את אמריקה: חידון חדש על ארץ האפשרויות הבלתי מוגבלות',
          link: 'http://rmkz.themarker.com/gampad/clk?id=37159514&iu=/9401/Haaretz.co.il.ClickTracker/Haaretz.co.il.ClickTracker.Button_222x108.4',
          linkTarget: '_blank',
          departments: [
            'haaretz',
          ],
          replaceDomainForAdBlocker: true,
          clicktrackerimage: {
            photographer: 'none',
            accessibility: 'טיולים מאורגנים',
            credit: 'פרסומת',
            aspects: {
              full: {
                width: 222,
                height: 108,
              },
            },
            imgArray: [
              {
                imgName: 'image/31693934.gif',
                version: '1517843919',
                aspects: {
                  full: {
                    width: 222,
                    height: 108,
                  },
                },
              },
            ],
            imageType: 'image',
            inputTemplate: 'com.tm.Image',
            contentId: '1.5790491',
            contentName: 'פגסוס',
          },
          advertiserCamp: 'פגסוס',
          percentage: 100,
          minRange: 0,
          maxRange: 100,
          chance: 1,
          inputTemplate: 'com.polobase.ClickTrackerBannerElement',
          contentId: '7.7379744',
          contentName: 'פגסוס חדש',
        },
      ],
      totalPercentage: 100,
      inputTemplate: 'com.polobase.ClickTrackerBannersWrapper',
      contentId: '7.3592183',
      contentName: '222/108 מיקום 4 - פגסוס',
    },
    {
      path: '/wellbeing/health-blogs/psychology/judithkatz/BLOG-1.5955035',
      kind: 'teaser',
      representedContent: '1.5955035',
      inputTemplate: 'com.tm.TeaserData',
      contentId: '7.7826482',
      exclusive: 'פסיכולוגיה חיובית',
      exclusiveMobile: 'פסיכולוגיה חיובית',
      title: 'עבד של הזמן: 10 שיטות לניהול זמן יעיל',
      titleMobile: 'עבד של הזמן: 10 שיטות לניהול זמן יעיל',
    },
    {
      id: 'haaretz.co.il.web.box.3',
      style: 'margin-bottom: 10px; margin-top: 10px;',
      className: 'ad h-hidden js-dfp-resp-refresh',
      audianceTarget: 'all',
      hideOnSite: false,
      inputTemplate: 'com.polobase.DfpBannerElement',
      contentId: '7.3620561',
      contentName: 'Haaretz.co.il.Web.Box.3',
    },
    {
      path: '/blogs/2.182307/leereuvenibardavid/BLOG-1.6633258',
      kind: 'teaser',
      representedContent: '1.6633258',
      inputTemplate: 'com.tm.TeaserData',
      contentId: '7.7825830',
      exclusive: 'מין זה שאיננו אחד',
      exclusiveMobile: 'מין זה שאיננו אחד',
      title: 'עוד על האח הגדול: חינוך מיני במשפחה',
      titleMobile: 'עוד על האח הגדול: חינוך מיני במשפחה',
    },
  ],
  inputTemplate: 'com.tm.element.List',
  contentId: '7.7829342',
  contentName: 'בריאות',
};
