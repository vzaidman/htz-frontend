...
const itemStyle = ({ theme, selected, lastItem, isSub, }) => ({
  ...
  ':hover, :focus': {
    backgroundColor: isSub ? theme.color('secondary', '+1') : theme.color('secondary', '+2'),
...
const arrowWrapperStyle = ({ theme, selected, }) => ({
  backgroundColor: selected ? theme.color('secondary', '+2') : theme.color('secondary', '+1'),
  position: 'relative',
  width: '4rem',
  ':focus': {
    backgroundColor: theme.color('secondary', '+2'),
...
const StyledLink = createComponent(linkStyle, Link, props => Object.keys(props));
...
const PromotionItem = createComponent(promotionStyle, Link, props => Object.keys(props));
...
class List extends React.Component {
  ...
  componentDidUpdate() {
    (this.state.selectedIndex === null && !this.props.items) && this.props.clearParentSelection();
  ...
  handleKeydown = e => {
    ...
    if (
      (this.props.theme.direrction === 'rtl' && key === 39) ||
      (this.props.theme.direrction === 'ltr' && key === 37)
    ) {
  ...
  addKeysListener() {
    this.listRef && this.listRef.addEventListener('keydown', this.handleKeydown);
  }

  removeKeysListener() {
    this.listRef && this.listRef.removeEventListener('keydown', this.handleKeydown);
  }
