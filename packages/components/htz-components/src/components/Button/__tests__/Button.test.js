...
describe('<Button>', () => {
  ...
  describe('DOM element', () => {
    ...
    testCase(
      'pass "id" to DOM element',
      <Button id="test">No one can take my ID</Button>
    );
    testCase(
      'render "busy" buttons correctly',
      <Button isBusy>No one can take my ID</Button>
    );
    testCase(
    ...
    testCase(
      'render "submit" buttons correctly',
      <Button isSubmit>I submit</Button>
    );
    testCase(
      'receive click handler',
      <Button onClick={evt => console.log('click')}>log</Button>
    );
    testCase('render custom tag', <Button tagName="div">Oh the shame</Button>);
  });
  describe('styles', () => {
    testCase(
      'modify "boxModel" (padding)',
      <Button boxModel={{ hp: 8, vp: 3, }}>padded</Button>
    );
    testCase(
    ...
    testCase(
      ...
      <Button isFull={[ { until: 'xl', value: true, }, ]}>
        Sometimes full-width
      </Button>
    );
    ...
    testCase(
      ...
      <Button varian="negative">Why so negative</Button>
    );
    testCase(
      'render different color variants in different breakpoints',
      <Button
        variant={[
          { until: 'xl', value: 'negative', },
          { from: 'xl', value: 'negativeOpaque', },
        ]}
      >
        Mood swings
      </Button>
