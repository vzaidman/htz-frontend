...
describe('Simple Time component', () => {
  it('renders correctly with default format DD.MM.YYYY HH:mm', () => {
    const snapshot = felaSnapshotter(<Time time="Wed, 31 Jan 2018 13:24:04 GMT" />);
    expect(snapshot).toMatchSnapshot();
  });

  it('renders correctly with format prop', () => {
    const snapshot = felaSnapshotter(<Time time="01/02/2018 16:30" format="DD-MM-YYYY hh:mm" />);
    expect(snapshot).toMatchSnapshot();
  });
});
