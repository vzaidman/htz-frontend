...
describe('<IconWhatsapp />', () => {
  ...
  //     expect(() => felaSnapshotter(<IconWhatsapp size="2rem" />)).toThrow(
  ...
  //     expect(() => felaSnapshotter(<IconWhatsapp color="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "color" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<IconWhatsapp color={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (string)', () => {
  //     expect(() => felaSnapshotter(<IconWhatsapp fill="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<IconWhatsapp fill={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  // });
  describe('DOM element', () => {
    testCase('render correctly when no props are passed', <IconWhatsapp />);
    testCase(
      'pass attributes defined in the "attrs" prop to the DOM element',
      <IconWhatsapp attrs={{ 'aria-hidden': true, }} />
    );
    testCase('add "id" attribute to DOM element', <IconWhatsapp id="hasId" />);
    testCase(
      'add "onClick" attribute to DOM element',
      <IconWhatsapp id={e => console.log('a')} />
    );
  });

  describe('styles', () => {
    testCase("set icon's color", <IconWhatsapp color={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's fill", <IconWhatsapp fill={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's size", <IconWhatsapp size={6} />);
    testCase(
    ...
    testCase(
      ...
      <IconWhatsapp miscStyles={{ position: 'relative', }} />
    );
    testCase(
