...
import IconTheMarker from '../IconTheMarker';

describe('<IconTheMarker />', () => {
  describe('Errors', () => {
    it('throw an error when the value passed to the "size" prop isn\'t a number', () => {
      expect(() => felaSnapshotter(<IconTheMarker size="2rem" />)).toThrow(
    ...
    it('throw an error when the value passed to the "color" prop isn\'t a named color (string)', () => {
      expect(() => felaSnapshotter(<IconTheMarker color="red" />)).toThrow(
        /is not a named-color/
      );
    });
    it('throw an error when the value passed to the "color" prop isn\'t a named color (array of strings)', () => {
      expect(() =>
        felaSnapshotter(<IconTheMarker color={[ 'primary', 'nope', ]} />)
      ).toThrow(/is not a named-color/);
    });
    it('throw an error when the value passed to the "fill" prop isn\'t a named color (string)', () => {
      expect(() => felaSnapshotter(<IconTheMarker fill="red" />)).toThrow(
        /is not a named-color/
      );
    });
    it('throw an error when the value passed to the "fill" prop isn\'t a named color (array of strings)', () => {
      expect(() =>
        felaSnapshotter(<IconTheMarker fill={[ 'primary', 'nope', ]} />)
      ).toThrow(/is not a named-color/);
    });
  });
  describe('DOM element', () => {
    testCase('render correctly when no props are passed', <IconTheMarker />);
    testCase(
      'pass attributes defined in the "attrs" prop to the DOM element',
      <IconTheMarker attrs={{ 'aria-hidden': true, }} />
    );
    testCase('add "id" attribute to DOM element', <IconTheMarker id="hasId" />);
    testCase(
      'add "onClick" attribute to DOM element',
      <IconTheMarker id={e => console.log('a')} />
    );
  });

  describe('styles', () => {
    testCase("set icon's color", <IconTheMarker color={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's fill", <IconTheMarker fill={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's size", <IconTheMarker size={6} />);
    testCase(
    ...
    testCase(
      ...
      <IconTheMarker miscStyles={{ position: 'relative', }} />
    );
    testCase(
