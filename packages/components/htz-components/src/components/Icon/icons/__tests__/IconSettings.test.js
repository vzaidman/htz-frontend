...
describe('<IconSettings />', () => {
  ...
  //     expect(() => felaSnapshotter(<IconSettings size="2rem" />)).toThrow(
  ...
  //     expect(() => felaSnapshotter(<IconSettings color="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "color" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<IconSettings color={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (string)', () => {
  //     expect(() => felaSnapshotter(<IconSettings fill="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<IconSettings fill={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  // });
  describe('DOM element', () => {
    testCase('render correctly when no props are passed', <IconSettings />);
    testCase(
      'pass attributes defined in the "attrs" prop to the DOM element',
      <IconSettings attrs={{ 'aria-hidden': true, }} />
    );
    testCase('add "id" attribute to DOM element', <IconSettings id="hasId" />);
    testCase(
      'add "onClick" attribute to DOM element',
      <IconSettings id={e => console.log('a')} />
    );
  });

  describe('styles', () => {
    testCase("set icon's color", <IconSettings color={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's fill", <IconSettings fill={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's size", <IconSettings size={6} />);
    testCase(
    ...
    testCase(
      ...
      <IconSettings miscStyles={{ position: 'relative', }} />
    );
    testCase(
