...
describe('<IconFacebook />', () => {
  ...
  //     expect(() => felaSnapshotter(<IconFacebook size="2rem" />)).toThrow(
  ...
  //     expect(() => felaSnapshotter(<IconFacebook color="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "color" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<IconFacebook color={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (string)', () => {
  //     expect(() => felaSnapshotter(<IconFacebook fill="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<IconFacebook fill={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  // });
  describe('DOM element', () => {
    testCase('render correctly when no props are passed', <IconFacebook />);
    testCase(
      'pass attributes defined in the "attrs" prop to the DOM element',
      <IconFacebook attrs={{ 'aria-hidden': true, }} />
    );
    testCase('add "id" attribute to DOM element', <IconFacebook id="hasId" />);
    testCase(
      'add "onClick" attribute to DOM element',
      <IconFacebook id={e => console.log('a')} />
    );
  });

  describe('styles', () => {
    testCase("set icon's color", <IconFacebook color={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's fill", <IconFacebook fill={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's size", <IconFacebook size={6} />);
    testCase(
    ...
    testCase(
      ...
      <IconFacebook miscStyles={{ position: 'relative', }} />
    );
    testCase(
