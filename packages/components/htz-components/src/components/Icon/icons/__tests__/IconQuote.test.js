...
describe('<IconQuote />', () => {
  ...
  //     expect(() => felaSnapshotter(<IconQuote size="2rem" />)).toThrow(
  ...
  //     expect(() => felaSnapshotter(<IconQuote color="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "color" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<IconQuote color={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (string)', () => {
  //     expect(() => felaSnapshotter(<IconQuote fill="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<IconQuote fill={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  // });
  describe('DOM element', () => {
    testCase('render correctly when no props are passed', <IconQuote />);
    testCase(
      'pass attributes defined in the "attrs" prop to the DOM element',
      <IconQuote attrs={{ 'aria-hidden': true, }} />
    );
    testCase('add "id" attribute to DOM element', <IconQuote id="hasId" />);
    testCase(
      'add "onClick" attribute to DOM element',
      <IconQuote id={e => console.log('a')} />
    );
  });

  describe('styles', () => {
    testCase("set icon's color", <IconQuote color={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's fill", <IconQuote fill={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's size", <IconQuote size={6} />);
    testCase(
      "set icon's size responsively",
      <IconQuote size={[ { until: 'xl', value: 6, }, { from: 'xl', value: 7, }, ]} />
    );
    testCase(
      'set misc styles on an icon',
      <IconQuote miscStyles={{ position: 'relative', }} />
    );
    testCase(
