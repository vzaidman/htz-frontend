...
describe('<IconAlef />', () => {
  ...
  //     expect(() => felaSnapshotter(<IconAlef size="2rem" />)).toThrow(
  ...
  describe('styles', () => {
    ...
    testCase("set icon's fill", <IconAlef fill={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's size", <IconAlef size={6} />);
    testCase(
      "set icon's size responsively",
      <IconAlef size={[ { until: 'xl', value: 6, }, { from: 'xl', value: 7, }, ]} />
    );
    testCase(
      'set misc styles on an icon',
      <IconAlef miscStyles={{ position: 'relative', }} />
    );
    testCase(
