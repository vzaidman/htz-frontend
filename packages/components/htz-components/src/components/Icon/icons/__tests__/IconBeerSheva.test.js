...
import IconBeerSheva from '../IconBeerSheva';

describe('<IconBeerSheva />', () => {
  ...
  //     expect(() => felaSnapshotter(<IconBeerSheva size="2rem" />)).toThrow(
  ...
  //     expect(() => felaSnapshotter(<IconBeerSheva color="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "color" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<IconBeerSheva color={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (string)', () => {
  //     expect(() => felaSnapshotter(<IconBeerSheva fill="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<IconBeerSheva fill={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  // });
  describe('DOM element', () => {
    testCase('render correctly when no props are passed', <IconBeerSheva />);
    testCase(
      'pass attributes defined in the "attrs" prop to the DOM element',
      <IconBeerSheva attrs={{ 'aria-hidden': true, }} />
    );
    testCase('add "id" attribute to DOM element', <IconBeerSheva id="hasId" />);
    testCase(
      'add "onClick" attribute to DOM element',
      <IconBeerSheva id={e => console.log('a')} />
    );
  });

  describe('styles', () => {
    testCase("set icon's color", <IconBeerSheva color={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's fill", <IconBeerSheva fill={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's size", <IconBeerSheva size={6} />);
    testCase(
    ...
    testCase(
      ...
      <IconBeerSheva miscStyles={{ position: 'relative', }} />
    );
    testCase(
