...
import IconLogoAlef from '../IconLogoAlef';

describe('<IconLogoAlef />', () => {
  ...
  //     expect(() => felaSnapshotter(<IconLogoAlef size="2rem" />)).toThrow(
  ...
  //     expect(() => felaSnapshotter(<IconLogoAlef color="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "color" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<IconLogoAlef color={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (string)', () => {
  //     expect(() => felaSnapshotter(<IconLogoAlef fill="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<IconLogoAlef fill={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  // });
  describe('DOM element', () => {
    testCase('render correctly when no props are passed', <IconLogoAlef />);
    testCase(
      'pass attributes defined in the "attrs" prop to the DOM element',
      <IconLogoAlef attrs={{ 'aria-hidden': true, }} />
    );
    testCase('add "id" attribute to DOM element', <IconLogoAlef id="hasId" />);
    testCase(
      'add "onClick" attribute to DOM element',
      <IconLogoAlef id={e => console.log('a')} />
    );
  });

  describe('styles', () => {
    testCase("set icon's color", <IconLogoAlef color={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's fill", <IconLogoAlef fill={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's size", <IconLogoAlef size={6} />);
    testCase(
    ...
    testCase(
      ...
      <IconLogoAlef miscStyles={{ position: 'relative', }} />
    );
    testCase(
