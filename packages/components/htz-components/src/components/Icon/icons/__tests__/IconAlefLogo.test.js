...
describe('<IconAlefLogo />', () => {
  ...
  //     expect(() => felaSnapshotter(<IconAlefLogo size="2rem" />)).toThrow(
  ...
  //     expect(() => felaSnapshotter(<IconAlefLogo color="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "color" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<IconAlefLogo color={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (string)', () => {
  //     expect(() => felaSnapshotter(<IconAlefLogo fill="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<IconAlefLogo fill={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  // });
  describe('DOM element', () => {
    testCase('render correctly when no props are passed', <IconAlefLogo />);
    testCase(
      'pass attributes defined in the "attrs" prop to the DOM element',
      <IconAlefLogo attrs={{ 'aria-hidden': true, }} />
    );
    testCase('add "id" attribute to DOM element', <IconAlefLogo id="hasId" />);
    testCase(
      'add "onClick" attribute to DOM element',
      <IconAlefLogo id={e => console.log('a')} />
    );
  });

  describe('styles', () => {
    testCase("set icon's color", <IconAlefLogo color={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's fill", <IconAlefLogo fill={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's size", <IconAlefLogo size={6} />);
    testCase(
    ...
    testCase(
      ...
      <IconAlefLogo miscStyles={{ position: 'relative', }} />
    );
    testCase(
