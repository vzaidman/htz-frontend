...
describe('<IconBold />', () => {
  ...
  //     expect(() => felaSnapshotter(<IconBold size="2rem" />)).toThrow(
  ...
  describe('styles', () => {
    ...
    testCase("set icon's fill", <IconBold fill={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's size", <IconBold size={6} />);
    testCase(
      "set icon's size responsively",
      <IconBold size={[ { until: 'xl', value: 6, }, { from: 'xl', value: 7, }, ]} />
    );
    testCase(
      'set misc styles on an icon',
      <IconBold miscStyles={{ position: 'relative', }} />
    );
    testCase(
