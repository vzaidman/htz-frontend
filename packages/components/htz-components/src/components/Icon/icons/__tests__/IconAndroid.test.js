...
describe('<IconAndroid />', () => {
  ...
  //     expect(() => felaSnapshotter(<IconAndroid size="2rem" />)).toThrow(
  ...
  //     expect(() => felaSnapshotter(<IconAndroid color="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "color" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<IconAndroid color={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (string)', () => {
  //     expect(() => felaSnapshotter(<IconAndroid fill="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<IconAndroid fill={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  // });
  describe('DOM element', () => {
    testCase('render correctly when no props are passed', <IconAndroid />);
    testCase(
      'pass attributes defined in the "attrs" prop to the DOM element',
      <IconAndroid attrs={{ 'aria-hidden': true, }} />
    );
    testCase('add "id" attribute to DOM element', <IconAndroid id="hasId" />);
    testCase(
      'add "onClick" attribute to DOM element',
      <IconAndroid id={e => console.log('a')} />
    );
  });

  describe('styles', () => {
    testCase("set icon's color", <IconAndroid color={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's fill", <IconAndroid fill={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's size", <IconAndroid size={6} />);
    testCase(
    ...
    testCase(
      ...
      <IconAndroid miscStyles={{ position: 'relative', }} />
    );
    testCase(
