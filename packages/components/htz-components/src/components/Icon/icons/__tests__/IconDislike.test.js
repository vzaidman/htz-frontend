...
describe('<IconDislike />', () => {
  describe('Errors', () => {
    it('throw an error when the value passed to the "size" prop isn\'t a number', () => {
      expect(() => felaSnapshotter(<IconDislike size="2rem" />)).toThrow(
    ...
    it('throw an error when the value passed to the "color" prop isn\'t a named color (string)', () => {
      expect(() => felaSnapshotter(<IconDislike color="red" />)).toThrow(
        /is not a named-color/
      );
    });
    it('throw an error when the value passed to the "color" prop isn\'t a named color (array of strings)', () => {
      expect(() =>
        felaSnapshotter(<IconDislike color={[ 'primary', 'nope', ]} />)
      ).toThrow(/is not a named-color/);
    });
    it('throw an error when the value passed to the "fill" prop isn\'t a named color (string)', () => {
      expect(() => felaSnapshotter(<IconDislike fill="red" />)).toThrow(
        /is not a named-color/
      );
    });
    it('throw an error when the value passed to the "fill" prop isn\'t a named color (array of strings)', () => {
      expect(() =>
        felaSnapshotter(<IconDislike fill={[ 'primary', 'nope', ]} />)
      ).toThrow(/is not a named-color/);
    });
  });
  describe('DOM element', () => {
    testCase('render correctly when no props are passed', <IconDislike />);
    testCase(
      'pass attributes defined in the "attrs" prop to the DOM element',
      <IconDislike attrs={{ 'aria-hidden': true, }} />
    );
    testCase('add "id" attribute to DOM element', <IconDislike id="hasId" />);
    testCase(
      'add "onClick" attribute to DOM element',
      <IconDislike id={e => console.log('a')} />
    );
  });

  describe('styles', () => {
    testCase("set icon's color", <IconDislike color={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's fill", <IconDislike fill={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's size", <IconDislike size={6} />);
    testCase(
    ...
    testCase(
      ...
      <IconDislike miscStyles={{ position: 'relative', }} />
    );
    testCase(
