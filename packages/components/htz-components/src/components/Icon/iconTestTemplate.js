...
describe('<${componentName} />', () => {
  ...
  //     expect(() => felaSnapshotter(<${componentName} size="2rem" />)).toThrow(
  ...
  //     expect(() => felaSnapshotter(<${componentName} color="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "color" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<${componentName} color={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (string)', () => {
  //     expect(() => felaSnapshotter(<${componentName} fill="red" />)).toThrow(
  //       /is not a named-color/
  //     );
  //   });
  //   it('throw an error when the value passed to the "fill" prop isn't a named color (array of strings)', () => {
  //     expect(() =>
  //       felaSnapshotter(<${componentName} fill={[ 'primary', 'nope', ]} />)
  //     ).toThrow(/is not a named-color/);
  //   });
  // });
  describe('DOM element', () => {
    testCase('render correctly when no props are passed', <${componentName} />);
    testCase(
      'pass attributes defined in the "attrs" prop to the DOM element',
      <${componentName} attrs={{ 'aria-hidden': true, }} />
    );
    testCase('add "id" attribute to DOM element', <${componentName} id="hasId" />);
    testCase(
      'add "onClick" attribute to DOM element',
      <${componentName} id={e => console.log('a')} />
    );
  });

  describe('styles', () => {
    testCase("set icon's color", <${componentName} color={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's fill", <${componentName} fill={[ 'primary', 'base', ]} />);
    testCase(
    ...
    testCase("set icon's size", <${componentName} size={6} />);
    testCase(
    ...
    testCase(
      ...
      <${componentName} miscStyles={{ position: 'relative', }} />
    );
    testCase(
