...
export default class ImpressionsManager {
  constructor(config) {
    ...
    this.config = Object.assign({}, config.impressionManagerConfig);
  ...
  saveImpressionsToLocalStorage() {
    if (this.debouncedSave && typeof this.debouncedSave === 'function') {
      ...
      this.debouncedSave = debounce(
        this.saveImpressionsToLocalStorageImpl,
        250,
        false
      );
      this.debouncedSave();
    ...
    Object.keys(this.config).map((key, index) => {
      ...
      if (slot) {
        ...
        if (this.config[adSlotId][keys.frequency] !== slot[keys.frequency]) {
          ...
          this.impressions[adSlotId][keys.frequency] = this.config[adSlotId][
            keys.frequency
          ];
        }
  ...
  updateExpiryDate(slotName) {
    ...
    if (
      !(
        this.impressions[slotName] && this.impressions[slotName][keys.frequency]
      )
    ) {
      throw new Error(
        `Unable to update expiry date for slot: ${slotName}
      - this.impressions[slotName]:`,
        this.impressions[slotName]
      );
    }
    const frequencyMap = this.impressions[slotName][keys.frequency].match(
      keys.frequencyRegex
    );
    ...
    this.impressions[slotName][keys.expires] = (frequencyMap.indexOf(
      keys.days
    ) > -1
      ? addDays(now, frequencyMap[2])
      : addHours(now, frequencyMap[2])
    ).getTime();

    // Set max impressions:
    this.impressions[slotName][keys.maxImpressions] = parseInt(
      frequencyMap[1],
      10
    );
    // Reset exposed
  ...
  reachedQuota(adSlotId) {
    ...
    const slotName = this.impressions[`${adSlotId}${globalConfig.department}`]
      ? `${adSlotId}${globalConfig.department}`
      : `${adSlotId}_all`;
