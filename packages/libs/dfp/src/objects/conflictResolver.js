...
class ConflictResolver {
  ...
  updateResolvedSlot(adSlotId, resolvedSize) {
    ...
    if (!resolvedSize) {
      throw new Error(
        'updateResolvedSlot must be called with a resolved size!'
      );
    }
    ...
    for (const adSlotKey of this.dependencyMap.keys()) {
      ...
      for (const adSlotRule of adSlot.rules) {
        ...
        if (adSlotRule.avoid === adSlotId) {
          ...
          if (
            adSlotRule.onsize.split(',').find(sizeString => {
              // eslint-disable-line
              const size = sizeString
                .split('x')
                .map(numberStr => parseInt(numberStr, 10));
              return isEqual(size, parentResolvedWith);
            })
          ) {
            // Block found
            this.deferredSlots.add(adSlotId);
  ...
  getBlockedSlotsIds(adSlotId) {
    ...
    if (this.dependencyMap.has(adSlotId)) {
      result = Array.from(
        this.dependencyMap.get(adSlotId).rules.map(adSlot => adSlot.avoid)
      );
    }
    return result || [];
  }
